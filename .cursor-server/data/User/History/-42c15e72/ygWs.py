...
async def save_user(telegram_id: int, phone: str, email: str, full_name: str) -> User:
    db = SessionLocal()
    try:
        user = User(
...
async def contact_handler(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        reply_markup=ReplyKeyboardMarkup(keyboard=[["Отмена"]], resize_keyboard=True)
    )
...
async def email_handler(message: Message, state: FSMContext):
    if message.text == "Отмена":
        await state.clear()
        await message.answer("Регистрация отменена. Нажмите /start для начала.", reply_markup=ReplyKeyboardMarkup(keyboard=[["/start"]], resize_keyboard=True))
        return
    ...
    try:
        ...
        if existing_user:
            await message.answer("Этот email уже используется. Введите другой email:")
            return
    finally:
        db.close()
...
async def name_handler(message: Message, state: FSMContext):
    ...
    user = await save_user(message.from_user.id, phone, email, full_name)
...
async def tariff_handler(message: Message):
    ...
    demo_config = f"vless://demo-user@{user.email}:443?type=tcp&security=reality&sni=example.com&fp=chrome&pbk=demo-key&sid=123456&spx=%2F#{user.email}"
