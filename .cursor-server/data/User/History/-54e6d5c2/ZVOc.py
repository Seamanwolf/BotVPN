    ...
    if message.text == "👤 Профиль":
        ...
        try:
            ...
            if user_subscriptions:
                ...
                print(f"🔍 Получаем конфигурацию для email: {user_email}")
                config = await xui_client.get_user_config(user_email)
                print(f"📋 Результат получения конфигурации: {config}")
                ...
                if active_subscription and config:
                    ...
                    if len(user_subscriptions) > 1:
                        manage_row = []
                        for sub in user_subscriptions:
                            sub_days_left = (sub.expires_at - datetime.utcnow()).days
                            if sub_days_left > 0:  # Подписка еще не истекла
...
async def tariff_handler(message: Message):
    ...
    try:
        ...
        print(f"🔧 Создаем пользователя в 3xUI: {user_email}, дней: {days}")
        xui_result = await xui_client.create_user(user_email, days, f"TG: {user.telegram_id} {user.full_name}", str(user.telegram_id))
        print(f"📋 Результат создания пользователя: {xui_result}")
        
        if xui_result and xui_result.get('success', False):
            # Получаем конфигурацию
            print(f"🔍 Получаем конфигурацию для: {user_email}")
            config = await xui_client.get_user_config(user_email)
            print(f"📋 Результат получения конфигурации: {config}")
