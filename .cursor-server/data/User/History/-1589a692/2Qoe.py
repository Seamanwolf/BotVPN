...
def get_next_key_number(user_id: int) -> int:
    """Получение следующего номера ключа для пользователя"""
    db = SessionLocal()
    try:
        # Получаем максимальный номер ключа для пользователя
        max_number = db.query(Subscription.key_number).filter(
            Subscription.user_id == user_id
        ).order_by(Subscription.key_number.desc()).first()
