...
class XUIClient:
    ...
    async def create_user(self, email: str, days: int = 30, note: str = "", tg_id: str = "") -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if existing_config:
                print(f"Пользователь с email {email} уже существует, удаляем старую запись")
                await self.delete_user(email)
            ...
            for inbound in inbounds["obj"]:
                ...
                for client in clients:
                    if client.get("email") == email:
                        ...
                        config = f"https://{server_host}/{XUI_WEBBASEPATH}/SeaMiniVpn-{tg_id}-1"
