            ...
            for inbound in inbounds["obj"]:
                ...
                for client in clients:
                    if client.get("email") == email:
                        # Формируем VLESS ссылку используя логику из fsfsdfsdf.txt
                        from config import XUI_BASE_URL
                        server_host = XUI_BASE_URL
                        client_id = client.get("id", "")
                        sub_id = client.get("subId", "")
                        
                        # Используем HTTPS для конечной ссылки (как в fsfsdfsdf.txt)
                        config = f"vless://{client_id}@{server_host}:443?security=tls&sni={server_host}&type=tcp#{email}"
