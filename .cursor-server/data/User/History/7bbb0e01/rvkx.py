...
def delete_user(user_id):
    ...
    try:
        ...
        try:
            ...
            # Удаляем все подписки пользователя из 3xUI
            for subscription in subscriptions:
                if subscription.status == "active":
                    # Получаем конфигурацию для удаления
                    config = loop.run_until_complete(xui_client.get_user_config(user_email))
                    if config:
                        # Удаляем пользователя из 3xUI
                        delete_result = loop.run_until_complete(xui_client.delete_user(user_email))
                        if delete_result:
                            print(f"Пользователь {user.telegram_id} удален из 3xUI")
                        else:
                            print(f"Ошибка удаления пользователя {user.telegram_id} из 3xUI")
                    break  # Удаляем только один раз, так как все подписки одного пользователя
            
            loop.close()
        except Exception as e:
            print(f"Ошибка при удалении из 3xUI: {e}")
            # Продолжаем удаление из БД даже если 3xUI недоступен
